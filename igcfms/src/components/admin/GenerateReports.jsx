...
const GenerateReports = ({ 
  ...
  const generateExcelReport = (payload) => {
    // Filter transactions based on date range and user role
    ...
    if (payload.date_from || payload.date_to) {
      reportTransactions = reportTransactions.filter(t => {
        ...
        if (payload.date_from && txDate < new Date(payload.date_from)) return false;
        if (payload.date_to) {
          const toDate = new Date(payload.date_to);
    ...
    if (payload.date_from || payload.date_to) {
      reportOverrides = reportOverrides.filter(o => {
        ...
        if (payload.date_from && oDate < new Date(payload.date_from)) return false;
        if (payload.date_to) {
          const toDate = new Date(payload.date_to);
          toDate.setHours(23, 59, 59, 999);
    ...
    const summaryData = [
      ...
      ['Role', user?.role || 'N/A'],
      ['Date From', payload.date_from || 'All'],
      ['Date To', payload.date_to || 'All'],
      [],
  ...
  const generateReportPDF = (report, allTransactions = []) => {
    ...
    let dayTransactions = allTransactions.filter(t => {
      const txDate = new Date(t.created_at || t.createdAt).toISOString().split('T')[0];
      return txDate === reportDate;
    });
