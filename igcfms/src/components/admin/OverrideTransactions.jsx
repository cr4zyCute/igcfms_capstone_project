...
const OverrideTransactions = ({ role }) => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        const res = await axios.get('http://localhost:8000/api/transactions', config);
  ...
  const handleReview = async (id, status) => {
    ...
    try {
      ...
      setOverrideRequests((prev) =>
        prev.map((req) =>
          req.id === id ? { ...req, status, review_notes: reviewNotes, reviewed_by: 'You' } : req
        )
      );
      setReviewNotes('');
    } catch (err) {
        ...
        <tbody>
          {overrideRequests.map((req) => (
            <tr key={req.id}>
              ...
              <td className="border p-2">{req.requested_by_name || req.requested_by}</td>
